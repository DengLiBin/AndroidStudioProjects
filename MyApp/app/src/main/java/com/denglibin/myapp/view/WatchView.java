package com.denglibin.myapp.view;
...
public class WatchView extends View{
    ...
    private float m_WatchCycleWidth=6;//表盘的外环宽度
    private float m_WatchRadius=200;//表盘的半径
    private float m_SecondCursorLength=180;//秒针的长度
    private float m_SecondCursorDegrees=0;//秒针旋转的角度
    private Path m_SecondCursorPath=new Path();//秒针的形状,通过Path(路径)来绘制

    private float m_MinuteCursorLength=120;//分针长度
    private float m_MinuteCursorDegrees=0;//分针旋转的角度
    private Path m_MinuteCursorPath=new Path();//分针的形状

    // 时钟的长度
    private float m_HourCursorLength = 100;
    // 时钟的形状
